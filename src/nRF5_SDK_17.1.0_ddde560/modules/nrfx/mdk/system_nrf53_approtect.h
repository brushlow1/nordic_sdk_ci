...
/* Function that handles firmware-driven enabling or disabling of APPROTECT on
   ...
   opened. If ENABLE_APPROTECT_USER_HANDLING is defined, the FW will not write
   to the fw branch of the APPROTECT mechanism. This allows later stages of the
   fw to handle APPROTECT, for example to implement authenticated debug.
...
static inline void nrf53_handle_approtect(void) {
  ...
  /* Do nothing, allow user code to handle APPROTECT. Use this if you want to
...
/* Secure APPROTECT is only available for Application core. */
  ...
  /* Prevent processor from unlocking SECURE APPROTECT soft branch after this
   * point. */
  NRF_CTRLAP_S->SECUREAPPROTECT.LOCK =
      NRF_CTRLAP_S_SECUREAPPROTECT_LOCK_LOCK_Locked;

#elif defined(ENABLE_SECURE_APPROTECT_USER_HANDLING)
  /* Do nothing, allow user code to handle SECURE APPROTECT. Use this if you
   * want to enable authenticated debug. */

#else
  /* Load SECURE APPROTECT soft branch from UICR.
     If UICR->SECUREAPPROTECT is disabled, CTRLAP->SECUREAPPROTECT will be
     disabled. */
