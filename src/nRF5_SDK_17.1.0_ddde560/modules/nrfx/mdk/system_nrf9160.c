...
#define __SYSTEM_CLOCK                                                         \
  (64000000UL) /*!< nRF9160 Application core uses a fixed System Clock         \
                  Frequency of 64MHz */
 ...
 * Nordic SPU should handle Secure Attribution tasks */
  ...
  /* Workaround for Errata 6 "POWER: SLEEPENTER and SLEEPEXIT events asserted
     after pin reset" found at the Errata document for your device located at
  ...
  /* Workaround for Errata 14 "REGULATORS: LDO mode at startup" found at the
  ...
  /* Workaround for Errata 15 "REGULATORS: LDO mode at startup" found at the
     Errata document for your device located at
     https://infocenter.nordicsemi.com/index.jsp  */
  if (nrf91_errata_15()) {
    NRF_REGULATORS_S->DCDCEN = REGULATORS_DCDCEN_DCDCEN_Enabled
  ...
  /* Workaround for Errata 20 "RAM content cannot be trusted upon waking up from
     System ON Idle or System OFF mode" found at the Errata document for your
     device located at https://infocenter.nordicsemi.com/index.jsp  */
  if (nrf91_errata_20()) {
    *((volatile uint32_t *)0x5003AEE4) = 0xE;
  }

  /* Workaround for Errata 31 "XOSC32k Startup Failure" found at the Errata
     document for your device located at
     https://infocenter.nordicsemi.com/index.jsp  */
  if (nrf91_errata_31()) {
  ...
  uint32_t index = 0;
  for (index = 0;
       index < 256ul && NRF_FICR_S->TRIMCNF[index].ADDR != 0xFFFFFFFFul;
       index++) {
...
#pragma diag_default = Pa082
  ...
  if (uicr_HFXOSRC_erased() || uicr_HFXOCNT_erased()) {
    ...
    if (uicr_HFXOSRC_erased()) {
      ...
      uicr_new_value = (uicr_erased_value & ~UICR_HFXOSRC_HFXOSRC_Msk) |
...
/* Enable Trace functionality. If ENABLE_TRACE is not defined, TRACE pins will
  ...
  *((uint32_t *)(0xE0058FB0ul)) = 0x00000000ul;
   ...
   * Some debug probes may require manual configuration of these peripherals to
   * enable tracing. ETB_BASE = 0xE0051000 TPIU_BASE = 0xE0054000
   */
#endif
