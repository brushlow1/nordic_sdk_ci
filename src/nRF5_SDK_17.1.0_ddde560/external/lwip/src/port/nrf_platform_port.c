 ...
 * NRF_DRIVER_ENABLE_LOGS.
 * @note If NRF_LWIP_DRIVER_CONFIG_LOG_ENABLED is disabled, having
 * NRF_DRIVER_ENABLE_LOGS has no effect.
 * @{
 */
...
/** Driver table to maintain mapping between IP interface and Bluetooth as
 * transport. */
struct blenetif {
...
struct blenetif
    m_blenetif_table[BLE_6LOWPAN_MAX_INTERFACE]; /**< Table maintaining network
                                                    interface of LwIP and also
                                                    corresponding 6lowpan
                                                    interface. */

/** @brief Function to add received packet to pbuf queue and notify the stack of
 * received packet. */
static void blenetif_input(struct netif *p_netif, uint8_t *p_payload,
...
/** @brief Network interface output function registered with LwIP to send
 * packets on 6lowpan. */
static err_t blenetif_output(struct netif *p_netif, struct pbuf *p_buffer,
                             const ip6_addr_t *p_addr) {
...
static void blenetif_transport_callback(iot_interface_t *p_interface,
  ...
  // Search network interface added for the 6lowpan interface that has generated
  // the event.
  ...
  case BLE_6LO_EVT_INTERFACE_ADD: {
    ...
    memcpy(p_blenetif->netif.hwaddr + 3, p_interface->local_addr.identifier + 5,
           3);
#else
    memcpy(p_blenetif->netif.hwaddr, p_interface->local_addr.identifier,
           IPV6_LL_ADDR_SIZE);
#endif
    p_blenetif->netif.hwaddr_len = IPV6_LL_ADDR_SIZE;

    UNUSED_VARIABLE(
        netif_add(&p_blenetif->netif, p_blenetif, blenetif_init, NULL));

    p_blenetif->netif.ip6_addr[0].addr[0] = HTONL(0xFE800000);
    p_blenetif->netif.ip6_addr[0].addr[1] = HTONL(0x00000000);
    p_blenetif->netif.ip6_addr[0].addr[2] =
        *((u32_t *)(&p_interface->local_addr.identifier[0]));
    p_blenetif->netif.ip6_addr[0].addr[3] =
        *((u32_t *)(&p_interface->local_addr.identifier[4]));
...
void nrf_message(const char *m) { NRF_DRIVER_LOG(m); }

/**@brief Interface implementation expected by the LwIP stack to get system
 * ticks. */
u32_t sys_now(void) {
  u32_t sys_tick;
  ...
  // Note: In case prescaler value is changed, this calculation to get sys tick
  // applies. Currently, this is not used for saving in code execution as
  // sys_now is called often.

  // sys_tick = ROUNDED_DIV(((sys_tick & 0x00FFFFF) *
  // (NRF_DRIVER_TIMER_PRESCALER + 1) * 1000), APP_TIMER_CLOCK_FREQ); sys_tick
  // *= 8;
  return sys_tick;
}
...
void *mem_trim(void *mem, mem_size_t size) { return nrf_realloc(mem, size); }

void *mem_malloc(mem_size_t size) { return nrf_malloc(size); }

void *mem_calloc(mem_size_t count, mem_size_t size) {
  return nrf_calloc(count, size);
}

void mem_free(void *mem) { nrf_free(mem); }
