...
int mbedtls_cipher_set_iv(mbedtls_cipher_context_t *ctx,
  ...
  if (ctx->cipher_info->type == MBEDTLS_CIPHER_CHACHA20) {
    if (0 !=
        mbedtls_chacha20_starts((mbedtls_chacha20_context *)ctx->cipher_ctx, iv,
...
int mbedtls_cipher_update_ad(mbedtls_cipher_context_t *ctx,
  ...
  if (MBEDTLS_MODE_GCM == ctx->cipher_info->mode) {
    return (mbedtls_gcm_starts((mbedtls_gcm_context *)ctx->cipher_ctx,
                               ctx->operation, ctx->iv, ctx->iv_size, ad,
                               ad_len));
  }
#endif
...
int mbedtls_cipher_update(mbedtls_cipher_context_t *ctx,
  ...
  *olen = 0;
  ...
  if (ctx->cipher_info->mode == MBEDTLS_MODE_GCM) {
    *olen = ilen;
    return (mbedtls_gcm_update((mbedtls_gcm_context *)ctx->cipher_ctx, ilen,
                               input, output));
  }
#endif
  ...
  if (ctx->cipher_info->mode == MBEDTLS_MODE_STREAM) {
    if (0 != (ret = ctx->cipher_info->base->stream_func(ctx->cipher_ctx, ilen,
                                                        input, output))) {
      return (ret);
    }
  ...
  *olen = 0;
  ...
  if (MBEDTLS_MODE_CBC == ctx->cipher_info->mode) {
    ...
    if (MBEDTLS_DECRYPT == ctx->operation)
      return (
          ctx->get_padding(output, mbedtls_cipher_get_block_size(ctx), olen));
...
#if defined(MBEDTLS_GCM_C)
  if (MBEDTLS_MODE_GCM == ctx->cipher_info->mode)
    return (mbedtls_gcm_finish((mbedtls_gcm_context *)ctx->cipher_ctx, tag,
                               tag_len));
#endif
...
int mbedtls_cipher_auth_encrypt(mbedtls_cipher_context_t *ctx,
  ...
  if (MBEDTLS_MODE_CCM == ctx->cipher_info->mode) {
    *olen = ilen;
    return (mbedtls_ccm_encrypt_and_tag(ctx->cipher_ctx, ilen, iv, iv_len, ad,
...
int mbedtls_cipher_auth_decrypt(mbedtls_cipher_context_t *ctx,
  ...
  if (MBEDTLS_CIPHER_CHACHA20_POLY1305 == ctx->cipher_info->type) {
    ...
    *olen = ilen;
    ret = mbedtls_chachapoly_auth_decrypt(ctx->cipher_ctx, ilen, iv, ad, ad_len,
                                          tag, input, output);
