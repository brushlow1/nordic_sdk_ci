...
static int ssl_double_retransmit_timeout(mbedtls_ssl_context *ssl) {
  ...
  if (ssl->handshake->retransmit_timeout != ssl->conf->hs_timeout_min) {
    ...
    MBEDTLS_SSL_DEBUG_MSG(
        2, ("mtu autoreduction to %d bytes", ssl->handshake->mtu));
  }
...
int (*mbedtls_ssl_hw_record_init)(
    mbedtls_ssl_context *ssl, const unsigned char *key_enc,
    const unsigned char *key_dec, size_t keylen, const unsigned char *iv_enc,
    const unsigned char *iv_dec, size_t ivlen, const unsigned char *mac_enc,
    const unsigned char *mac_dec, size_t maclen) = NULL;
...
static int tls_prf_sha256(const unsigned char *secret, size_t slen,
                          const char *label, const unsigned char *random,
                          size_t rlen, unsigned char *dstbuf, size_t dlen) {
  return (tls_prf_generic(MBEDTLS_MD_SHA256, secret, slen, label, random, rlen,
                          dstbuf, dlen));
}
...
static int tls_prf_sha384(const unsigned char *secret, size_t slen,
                          const char *label, const unsigned char *random,
                          size_t rlen, unsigned char *dstbuf, size_t dlen) {
  return (tls_prf_generic(MBEDTLS_MD_SHA384, secret, slen, label, random, rlen,
                          dstbuf, dlen));
}
...
static void ssl_update_checksum_md5sha1(mbedtls_ssl_context *,
                                        const unsigned char *, size_t);
#endif
...
int mbedtls_ssl_derive_keys(mbedtls_ssl_context *ssl) {
  ...
  cipher_info =
      mbedtls_cipher_info_from_type(transform->ciphersuite_info->cipher);
  if (cipher_info == NULL) {
  ...
  if (handshake->resume == 0) {
    ...
    if (ssl->handshake->extended_ms == MBEDTLS_SSL_EXTENDED_MS_ENABLED) {
      ...
      ret = handshake->tls_prf(handshake->premaster, handshake->pmslen,
                               "extended master secret", session_hash, hash_len,
                               session->master, 48);
      if (ret != 0) {
...
#endif
      ret = handshake->tls_prf(handshake->premaster, handshake->pmslen,
                               "master secret", handshake->randbytes, 64,
                               session->master, 48);
    if (ret != 0) {
      MBEDTLS_SSL_DEBUG_RET(1, "prf", ret);
      return (ret);
  ...
  if (cipher_info->mode == MBEDTLS_MODE_GCM ||
    ...
    taglen = transform->ciphersuite_info->flags & MBEDTLS_CIPHERSUITE_SHORT_TAG
                 ? 8
                 : 16;
  ...
  MBEDTLS_SSL_DEBUG_MSG(3, ("keylen: %d, minlen: %d, ivlen: %d, maclen: %d",
                            transform->keylen, transform->minlen,
                            transform->ivlen, transform->maclen));
  ...
  if (ssl->conf->f_export_keys != NULL) {
    ssl->conf->f_export_keys(ssl->conf->p_export_keys, session->master, keyblk,
                             mac_key_len, transform->keylen, iv_copy_len);
  }
#endif

  if ((ret = mbedtls_cipher_setup(&transform->cipher_ctx_enc, cipher_info)) !=
      0) {
    MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_cipher_setup", ret);
    return (ret);
  }

  if ((ret = mbedtls_cipher_setup(&transform->cipher_ctx_dec, cipher_info)) !=
      0) {
    MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_cipher_setup", ret);
    return (ret);
  }
  ...
  if (session->compression == MBEDTLS_SSL_COMPRESS_DEFLATE) {
    if (ssl->compress_buf == NULL) {
      ...
      if (ssl->compress_buf == NULL) {
        MBEDTLS_SSL_DEBUG_MSG(
            1, ("alloc(%d bytes) failed", MBEDTLS_SSL_COMPRESS_BUFFER_LEN));
        return (MBEDTLS_ERR_SSL_ALLOC_FAILED);
      }
    }
...
int mbedtls_ssl_psk_derive_premaster(mbedtls_ssl_context *ssl,
  ...
  if (key_ex == MBEDTLS_KEY_EXCHANGE_PSK) {
    ...
    *p++ = 48;
    ...
    if ((ret = mbedtls_dhm_calc_secret(&ssl->handshake->dhm_ctx, p + 2,
                                       end - (p + 2), &len, ssl->conf->f_rng,
                                       ssl->conf->p_rng)) != 0) {
...
static void ssl_mac(mbedtls_md_context_t *md_ctx, const unsigned char *secret,
...
static int ssl_encrypt_buf(mbedtls_ssl_context *ssl) {
  ...
  MBEDTLS_SSL_DEBUG_BUF(4, "before encrypt: output payload", ssl->out_msg,
                        ssl->out_msglen);
  ...
  if (mode == MBEDTLS_MODE_STREAM ||
    ...
    if (ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_0) {
      ...
      ssl_mac(&ssl->transform_out->md_ctx_enc, ssl->transform_out->mac_enc,
              ssl->out_msg, ssl->out_msglen, ssl->out_ctr, ssl->out_msgtype,
              mac);
    ...
    defined(MBEDTLS_SSL_PROTO_TLS1_2)
      ...
      mbedtls_md_hmac_update(&ssl->transform_out->md_ctx_enc, ssl->out_msg,
                             ssl->out_msglen);
    ...
    MBEDTLS_SSL_DEBUG_BUF(4, "computed mac", ssl->out_msg + ssl->out_msglen,
  ...
  if (mode == MBEDTLS_MODE_STREAM) {
    ...
    if ((ret = mbedtls_cipher_crypt(
             &ssl->transform_out->cipher_ctx_enc, ssl->transform_out->iv_enc,
             ssl->transform_out->ivlen, ssl->out_msg, ssl->out_msglen,
             ssl->out_msg, &olen)) != 0) {
    ...
    unsigned char taglen =
        transform->ciphersuite_info->flags & MBEDTLS_CIPHERSUITE_SHORT_TAG ? 8
                                                                           : 16;
    ...
    MBEDTLS_SSL_DEBUG_BUF(4, "IV used (transmitted)", ssl->out_iv,
                          explicit_ivlen);
    ...
    if ((ret = mbedtls_cipher_auth_encrypt(
             &transform->cipher_ctx_enc, iv, transform->ivlen, add_data, 13,
             enc_msg, enc_msglen, enc_msg, &olen, enc_msg + enc_msglen,
             taglen)) != 0) {
    ...
    padlen = ssl->transform_out->ivlen -
             (ssl->out_msglen + 1) % ssl->transform_out->ivlen;
    ...
    if ((ret = mbedtls_cipher_crypt(&ssl->transform_out->cipher_ctx_enc,
                                    ...
                                    ssl->transform_out->ivlen, enc_msg,
    ...
    if (ssl->minor_ver < MBEDTLS_SSL_MINOR_VERSION_2) {
       ...
       */
      memcpy(ssl->transform_out->iv_enc, ssl->transform_out->cipher_ctx_enc.iv,
             ssl->transform_out->ivlen);
    }
#endif
    ...
    if (auth_done == 0) {
      ...
      mbedtls_md_hmac_update(&ssl->transform_out->md_ctx_enc, ssl->out_iv,
                             ssl->out_msglen);
      mbedtls_md_hmac_finish(&ssl->transform_out->md_ctx_enc, mac);
      mbedtls_md_hmac_reset(&ssl->transform_out->md_ctx_enc);
...
static int ssl_decrypt_buf(mbedtls_ssl_context *ssl) {
  ...
  if (ssl->in_msglen < ssl->transform_in->minlen) {
    MBEDTLS_SSL_DEBUG_MSG(1, ("in_msglen (%d) < minlen (%d)", ssl->in_msglen,
                              ssl->transform_in->minlen));
  ...
  if (mode == MBEDTLS_MODE_STREAM) {
    ...
    if ((ret = mbedtls_cipher_crypt(&ssl->transform_in->cipher_ctx_dec,
                                    ...
                                    ssl->transform_in->ivlen, ssl->in_msg,
    ...
    if ((ret = mbedtls_cipher_auth_decrypt(
             &ssl->transform_in->cipher_ctx_dec, iv, transform->ivlen, add_data,
             13, dec_msg, dec_msglen, dec_msg_result, &olen,
    ...
    if (ssl->session_in->encrypt_then_mac == MBEDTLS_SSL_ETM_ENABLED) {
      ...
      mbedtls_md_hmac_update(&ssl->transform_in->md_ctx_dec, ssl->in_iv,
                             ssl->in_msglen);
    ...
    if ((ret = mbedtls_cipher_crypt(&ssl->transform_in->cipher_ctx_dec,
                                    ssl->transform_in->iv_dec,
                                    ssl->transform_in->ivlen, dec_msg,
    ...
    if (ssl->minor_ver < MBEDTLS_SSL_MINOR_VERSION_2) {
       ...
       * Save IV in SSL3 and TLS1
       */
      memcpy(ssl->transform_in->iv_dec, ssl->transform_in->cipher_ctx_dec.iv,
    ...
    defined(MBEDTLS_SSL_PROTO_TLS1_2)
      ...
      correct &= (padding_idx <=
                  MBEDTLS_SSL_IN_CONTENT_LEN + ssl->transform_in->maclen);
...
#if defined(MBEDTLS_SSL_DEBUG_ALL)
  MBEDTLS_SSL_DEBUG_BUF(4, "raw buffer after decryption", ssl->in_msg,
                        ssl->in_msglen);
#endif
  ...
  if (auth_done == 0) {
...
#if defined(MBEDTLS_SSL_PROTO_SSL3)
    if (ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_0) {
      ssl_mac(&ssl->transform_in->md_ctx_dec, ssl->transform_in->mac_dec,
              ssl->in_msg, ssl->in_msglen, ssl->in_ctr, ssl->in_msgtype,
              mac_expect);
    ...
    defined(MBEDTLS_SSL_PROTO_TLS1_2)
      ...
      ret = mbedtls_ssl_cf_hmac(&ssl->transform_in->md_ctx_dec, add_data,
                                sizeof(add_data), ssl->in_msg, ssl->in_msglen,
      ...
      mbedtls_ssl_cf_memcpy_offset(mac_peer, ssl->in_msg, ssl->in_msglen,
                                   min_len, max_len, ssl->transform_in->maclen);
    } else
#endif /* MBEDTLS_SSL_PROTO_TLS1 || MBEDTLS_SSL_PROTO_TLS1_1 ||                \
              MBEDTLS_SSL_PROTO_TLS1_2 */
    {
    ...
    MBEDTLS_SSL_DEBUG_BUF(4, "expected mac", mac_expect,
                          ssl->transform_in->maclen);
    MBEDTLS_SSL_DEBUG_BUF(4, "message  mac", mac_peer,
                          ssl->transform_in->maclen);
#endif
  ...
  if (ssl->in_msglen == 0) {
    ...
    if (ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_3 &&
      ...
      /* TLS v1.2 explicitly disallows zero-length messages which are not
       * application data */
      MBEDTLS_SSL_DEBUG_MSG(
          1, ("invalid zero-length message type: %d", ssl->in_msgtype));
...
static int ssl_compress_buf(mbedtls_ssl_context *ssl) {
  ...
  MBEDTLS_SSL_DEBUG_MSG(3,
                        ("before compression: msglen = %d, ", ssl->out_msglen));

  MBEDTLS_SSL_DEBUG_BUF(4, "before compression: output payload", ssl->out_msg,
                        ssl->out_msglen);
  ...
  ssl->transform_out->ctx_deflate.avail_out =
      MBEDTLS_SSL_OUT_BUFFER_LEN - bytes_written;
  ...
  MBEDTLS_SSL_DEBUG_MSG(3,
                        ("after compression: msglen = %d, ", ssl->out_msglen));

  MBEDTLS_SSL_DEBUG_BUF(4, "after compression: output payload", ssl->out_msg,
                        ssl->out_msglen);
...
static int ssl_decompress_buf(mbedtls_ssl_context *ssl) {
  ...
  MBEDTLS_SSL_DEBUG_MSG(
      3, ("before decompression: msglen = %d, ", ssl->in_msglen));

  MBEDTLS_SSL_DEBUG_BUF(4, "before decompression: input payload", ssl->in_msg,
                        ssl->in_msglen);
  ...
  ssl->transform_in->ctx_inflate.avail_out =
      MBEDTLS_SSL_IN_BUFFER_LEN - header_bytes;
  ...
  MBEDTLS_SSL_DEBUG_MSG(3,
                        ("after decompression: msglen = %d, ", ssl->in_msglen));

  MBEDTLS_SSL_DEBUG_BUF(4, "after decompression: input payload", ssl->in_msg,
                        ssl->in_msglen);
...
int mbedtls_ssl_fetch_input(mbedtls_ssl_context *ssl, size_t nb_want) {
  ...
  if (nb_want >
      MBEDTLS_SSL_IN_BUFFER_LEN - (size_t)(ssl->in_hdr - ssl->in_buf)) {
...
#endif
  {
    ...
    while (ssl->in_left < nb_want) {
      ...
      MBEDTLS_SSL_DEBUG_MSG(
          2, ("in_left: %d, nb_want: %d", ssl->in_left, nb_want));
      MBEDTLS_SSL_DEBUG_RET(2, "ssl->f_recv(_timeout)", ret);
      ...
      if ((size_t)ret > len || (INT_MAX > SIZE_MAX && ret > (int)SIZE_MAX)) {
        MBEDTLS_SSL_DEBUG_MSG(
            1, ("f_recv returned %d bytes but only %lu were requested", ret,
                (unsigned long)len));
        return (MBEDTLS_ERR_SSL_INTERNAL_ERROR);
      }
...
static int ssl_flight_append(mbedtls_ssl_context *ssl) {
  ...
  if ((msg = mbedtls_calloc(1, sizeof(mbedtls_ssl_flight_item))) == NULL) {
    MBEDTLS_SSL_DEBUG_MSG(
        1, ("alloc %d bytes failed", sizeof(mbedtls_ssl_flight_item)));
    return (MBEDTLS_ERR_SSL_ALLOC_FAILED);
  }
...
static int ssl_swap_epochs(mbedtls_ssl_context *ssl) {
  ...
  if (mbedtls_ssl_hw_record_activate != NULL) {
    if ((ret = mbedtls_ssl_hw_record_activate(
             ssl, MBEDTLS_SSL_CHANNEL_OUTBOUND)) != 0) {
...
int mbedtls_ssl_write_record(mbedtls_ssl_context *ssl, uint8_t force_flush) {
  ...
  if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM &&
    ...
    if (remaining == 0) {
      ...
      MBEDTLS_SSL_DEBUG_MSG(2, ("Still %u bytes available in current datagram",
                                (unsigned)remaining));
    }
  }
#endif /* MBEDTLS_SSL_PROTO_DTLS */
  ...
  if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
    ...
    if (ssl->handshake != NULL &&
      ...
      if (recv_msg_seq > ssl->handshake->in_msg_seq) {
        MBEDTLS_SSL_DEBUG_MSG(2, ("received future handshake message of "
                                  "sequence number %u (next %u)",
                                  recv_msg_seq, ssl->handshake->in_msg_seq));
...
static int ssl_check_dtls_clihlo_cookie(
    ...
    const unsigned char *cli_id, size_t cli_id_len, const unsigned char *in,
    size_t in_len, unsigned char *obuf, size_t buf_len, size_t *olen) {
...
static int ssl_handle_possible_reconnect(mbedtls_ssl_context *ssl) {
  ...
  ret = ssl_check_dtls_clihlo_cookie(
      ...
      ssl->cli_id, ssl->cli_id_len, ssl->in_buf, ssl->in_left, ssl->out_buf,
      MBEDTLS_SSL_OUT_CONTENT_LEN, &len);
...
static int ssl_parse_record_header(mbedtls_ssl_context *ssl) {
  ...
  if (ssl->in_msglen >
      MBEDTLS_SSL_IN_BUFFER_LEN - (size_t)(ssl->in_msg - ssl->in_buf)) {
  ...
  if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
    ...
    if (rec_epoch != ssl->in_epoch) {
       ...
       */
      if (ssl->conf->endpoint == MBEDTLS_SSL_IS_SERVER &&
          ssl->state == MBEDTLS_SSL_HANDSHAKE_OVER && rec_epoch == 0 &&
  ...
  if (ssl->transform_in == NULL) {
    ...
    if (ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_0 &&
        ssl->in_msglen >
            ssl->transform_in->minlen + MBEDTLS_SSL_IN_CONTENT_LEN) {
      MBEDTLS_SSL_DEBUG_MSG(1, ("bad message length"));
      return (MBEDTLS_ERR_SSL_INVALID_RECORD);
    }
#endif
    ...
    if (ssl->minor_ver >= MBEDTLS_SSL_MINOR_VERSION_1 &&
        ssl->in_msglen >
            ssl->transform_in->minlen + MBEDTLS_SSL_IN_CONTENT_LEN + 256) {
      MBEDTLS_SSL_DEBUG_MSG(1, ("bad message length"));
      return (MBEDTLS_ERR_SSL_INVALID_RECORD);
    }
#endif
...
static int ssl_prepare_record_content(mbedtls_ssl_context *ssl) {
  ...
  MBEDTLS_SSL_DEBUG_BUF(4, "input record from network", ssl->in_hdr,
                        mbedtls_ssl_hdr_len(ssl) + ssl->in_msglen);
  ...
  if (!done && ssl->transform_in != NULL) {
    ...
    MBEDTLS_SSL_DEBUG_BUF(4, "input payload after decrypt", ssl->in_msg,
                          ssl->in_msglen);
...
static int ssl_load_buffered_message(mbedtls_ssl_context *ssl) {
  ...
  if ((hs_buf->is_valid == 1) && (hs_buf->is_complete == 1)) {
    ...
    size_t msg_len =
        (hs_buf->data[1] << 16) | (hs_buf->data[2] << 8) | hs_buf->data[3];
...
static int ssl_buffer_make_space(mbedtls_ssl_context *ssl, size_t desired) {
  ...
  for (offset = MBEDTLS_SSL_MAX_BUFFERED_HS - 1; offset >= 0; offset--) {
    MBEDTLS_SSL_DEBUG_MSG(2, ("Free buffering slot %d to make space for "
                              "reassembly of next handshake message",
                              offset));
    ...
    if (desired <=
        (MBEDTLS_SSL_DTLS_MAX_BUFFERING - hs->buffering.total_bytes_buffered)) {
      MBEDTLS_SSL_DEBUG_MSG(
          2, ("Enough space available after freeing buffered HS messages"));
      return (0);
    }
  }
...
static int ssl_buffer_message(mbedtls_ssl_context *ssl) {
  ...
  case MBEDTLS_SSL_MSG_HANDSHAKE: {
    ...
    MBEDTLS_SSL_DEBUG_MSG(
        2, ("Buffering HS message with sequence number %u, offset %u ",
    ...
    if (!hs_buf->is_valid) {
      ...
      reassembly_buf_sz =
          ssl_get_reassembly_buffer_size(msg_len, hs_buf->is_fragmented);
      ...
      if (reassembly_buf_sz > (MBEDTLS_SSL_DTLS_MAX_BUFFERING -
        ...
        if (recv_msg_seq_offset > 0) {
          ...
          MBEDTLS_SSL_DEBUG_MSG(
              2,
              ("Buffering of future message of size %u would exceed the "
               "compile-time limit %u (already %u bytes buffered) -- ignore\n",
               (unsigned)msg_len, MBEDTLS_SSL_DTLS_MAX_BUFFERING,
               (unsigned)hs->buffering.total_bytes_buffered));
          goto exit;
        } else {
          MBEDTLS_SSL_DEBUG_MSG(
              2, ("Buffering of future message of size %u would exceed the "
                  "compile-time limit %u (already %u bytes buffered) -- "
                  "attempt to make space by freeing buffered future messages\n",
                  (unsigned)msg_len, MBEDTLS_SSL_DTLS_MAX_BUFFERING,
                  (unsigned)hs->buffering.total_bytes_buffered));
        }
        ...
        if (ssl_buffer_make_space(ssl, reassembly_buf_sz) != 0) {
          MBEDTLS_SSL_DEBUG_MSG(
              2, ("Reassembly of next message of size %u (%u with bitmap) "
                  "would exceed the compile-time limit %u (already %u bytes "
                  "buffered) -- fail\n",
      ...
      MBEDTLS_SSL_DEBUG_MSG(
          2, ("initialize reassembly, total length = %d", msg_len));
    ...
    if (!hs_buf->is_complete) {
      ...
      MBEDTLS_SSL_DEBUG_MSG(
          2, ("adding fragment, offset = %d, length = %d", frag_off, frag_len));
...
static int ssl_buffer_future_record(mbedtls_ssl_context *ssl) {
  ...
  if (total_buf_sz >
      (MBEDTLS_SSL_DTLS_MAX_BUFFERING - hs->buffering.total_bytes_buffered)) {
    MBEDTLS_SSL_DEBUG_MSG(
        2, ("Buffering of future epoch record of size %u would exceed the "
            "compile-time limit %u (already %u bytes buffered) -- ignore\n",
...
int mbedtls_ssl_handle_message_type(mbedtls_ssl_context *ssl) {
  ...
  if (ssl->in_msgtype == MBEDTLS_SSL_MSG_CHANGE_CIPHER_SPEC) {
    if (ssl->in_msglen != 1) {
      MBEDTLS_SSL_DEBUG_MSG(1,
                            ("invalid CCS message, len: %d", ssl->in_msglen));
      return (MBEDTLS_ERR_SSL_INVALID_RECORD);
    }

    if (ssl->in_msg[0] != 1) {
      MBEDTLS_SSL_DEBUG_MSG(
          1, ("invalid CCS message, content: %02x", ssl->in_msg[0]));
      return (MBEDTLS_ERR_SSL_INVALID_RECORD);
    }
  ...
  if (ssl->in_msgtype == MBEDTLS_SSL_MSG_ALERT) {
    if (ssl->in_msglen != 2) {
      ...
      MBEDTLS_SSL_DEBUG_MSG(1,
                            ("invalid alert message, len: %d", ssl->in_msglen));
      return (MBEDTLS_ERR_SSL_INVALID_RECORD);
    }
    ...
    if (ssl->in_msg[0] == MBEDTLS_SSL_ALERT_LEVEL_FATAL) {
      MBEDTLS_SSL_DEBUG_MSG(
          1, ("is a fatal alert message (msg %d)", ssl->in_msg[1]));
...
int mbedtls_ssl_write_certificate(mbedtls_ssl_context *ssl) {
  ...
  while (crt != NULL) {
    ...
    if (n > MBEDTLS_SSL_OUT_CONTENT_LEN - 3 - i) {
      MBEDTLS_SSL_DEBUG_MSG(1, ("certificate too large, %d > %d", i + 3 + n,
...
crt_verify:
  ...
  if (authmode != MBEDTLS_SSL_VERIFY_NONE) {
    ...
    if (mbedtls_ssl_check_cert_usage(ssl->session_negotiate->peer_cert,
                                     ciphersuite_info, !ssl->conf->endpoint,
...
int mbedtls_ssl_parse_change_cipher_spec(mbedtls_ssl_context *ssl) {
  ...
  if (mbedtls_ssl_hw_record_activate != NULL) {
    if ((ret = mbedtls_ssl_hw_record_activate(
             ssl, MBEDTLS_SSL_CHANNEL_INBOUND)) != 0) {
      MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_ssl_hw_record_activate", ret);
      mbedtls_ssl_send_alert_message(ssl, MBEDTLS_SSL_ALERT_LEVEL_FATAL,
                                     MBEDTLS_SSL_ALERT_MSG_INTERNAL_ERROR);
      return (MBEDTLS_ERR_SSL_HW_ACCEL_FAILED);
...
#if defined(MBEDTLS_SSL_PROTO_SSL3)
static void ssl_calc_finished_ssl(mbedtls_ssl_context *ssl, unsigned char *buf,
                                  int from) {
  const char *sender;
  mbedtls_md5_context md5;
  mbedtls_sha1_context sha1;
  ...
  MBEDTLS_SSL_DEBUG_BUF(4, "finished  md5 state", (unsigned char *)md5.state,
                        sizeof(md5.state));
#endif

#if !defined(MBEDTLS_SHA1_ALT)
  MBEDTLS_SSL_DEBUG_BUF(4, "finished sha1 state", (unsigned char *)sha1.state,
                        sizeof(sha1.state));
#endif
...
#if defined(MBEDTLS_SSL_PROTO_TLS1) || defined(MBEDTLS_SSL_PROTO_TLS1_1)
static void ssl_calc_finished_tls(mbedtls_ssl_context *ssl, unsigned char *buf,
                                  int from) {
  int len = 12;
  const char *sender;
  mbedtls_md5_context md5;
  ...
  sender =
      (from == MBEDTLS_SSL_IS_CLIENT) ? "client finished" : "server finished";
...
static void ssl_calc_finished_tls_sha256(mbedtls_ssl_context *ssl,
  ...
  MBEDTLS_SSL_DEBUG_BUF(4, "finished sha2 state", (unsigned char *)sha256.state,
                        sizeof(sha256.state));
#endif
...
int mbedtls_ssl_setup(mbedtls_ssl_context *ssl,
  ...
  if (ssl->in_buf == NULL) {
    MBEDTLS_SSL_DEBUG_MSG(
        1, ("alloc(%d bytes) failed", MBEDTLS_SSL_IN_BUFFER_LEN));
    ret = MBEDTLS_ERR_SSL_ALLOC_FAILED;
    goto error;
  }
  ...
  if (ssl->out_buf == NULL) {
    MBEDTLS_SSL_DEBUG_MSG(
        1, ("alloc(%d bytes) failed", MBEDTLS_SSL_OUT_BUFFER_LEN));
    ret = MBEDTLS_ERR_SSL_ALLOC_FAILED;
    goto error;
  }
...
void mbedtls_ssl_conf_session_cache(
    mbedtls_ssl_config *conf, void *p_cache,
...
void mbedtls_ssl_conf_session_tickets_cb(
    mbedtls_ssl_config *conf, mbedtls_ssl_ticket_write_t *f_ticket_write,
...
void mbedtls_ssl_conf_async_private_cb(
    mbedtls_ssl_config *conf, mbedtls_ssl_async_sign_t *f_async_sign,
...
int mbedtls_ssl_check_pending(const mbedtls_ssl_context *ssl) {
  ...
  if (ssl->in_offt != NULL) {
    MBEDTLS_SSL_DEBUG_MSG(
        3, ("ssl_check_pending: application data record is being processed"));
    return (1);
  }
 ...
 */
static int ssl_write_real(mbedtls_ssl_context *ssl, const unsigned char *buf,
                          size_t len) {
 ...
 */
#if defined(MBEDTLS_SSL_CBC_RECORD_SPLITTING)
static int ssl_write_split(mbedtls_ssl_context *ssl, const unsigned char *buf,
                           size_t len) {
  int ret;

  if (ssl->conf->cbc_record_splitting ==
          MBEDTLS_SSL_CBC_RECORD_SPLITTING_DISABLED ||
      len <= 1 || ssl->minor_ver > MBEDTLS_SSL_MINOR_VERSION_1 ||
...
#if defined(MBEDTLS_ZLIB_SUPPORT)
  if (ssl->compress_buf != NULL) {
    mbedtls_platform_zeroize(ssl->compress_buf,
...
int mbedtls_ssl_config_defaults(mbedtls_ssl_config *conf, int endpoint,
  ...
  default:
    conf->min_major_ver =
        (MBEDTLS_SSL_MIN_MAJOR_VERSION > MBEDTLS_SSL_MIN_VALID_MAJOR_VERSION)
            ? MBEDTLS_SSL_MIN_MAJOR_VERSION
            : MBEDTLS_SSL_MIN_VALID_MAJOR_VERSION;
    conf->min_minor_ver =
        (MBEDTLS_SSL_MIN_MINOR_VERSION > MBEDTLS_SSL_MIN_VALID_MINOR_VERSION)
...
int mbedtls_ssl_get_key_exchange_md_ssl_tls(mbedtls_ssl_context *ssl,
  ...
  if ((ret = mbedtls_md5_update_ret(&mbedtls_md5, ssl->handshake->randbytes,
                                    64)) != 0) {
  ...
  if ((ret = mbedtls_sha1_update_ret(&mbedtls_sha1, ssl->handshake->randbytes,
                                     64)) != 0) {
