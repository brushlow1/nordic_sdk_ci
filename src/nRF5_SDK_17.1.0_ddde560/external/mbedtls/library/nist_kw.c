 ...
 */
static int unwrap(mbedtls_nist_kw_context *ctx, const unsigned char *input,
                  size_t semiblocks, unsigned char A[KW_SEMIBLOCK_LENGTH],
                  unsigned char *output, size_t *out_len) {
  int ret = 0;
  ...
  if (mode == MBEDTLS_KW_MODE_KW) {
    ...
    if (in_len == KW_SEMIBLOCK_LENGTH * 2) {
      ...
      *out_len = KW_SEMIBLOCK_LENGTH;
      ...
      ret =
          unwrap(ctx, input, in_len / KW_SEMIBLOCK_LENGTH, A, output, out_len);
      if (ret != 0)
        goto cleanup;
    }
    ...
    /* Check ICV in "constant-time" */
    diff =
        mbedtls_nist_kw_safer_memcmp(NIST_KW_ICV2, A, KW_SEMIBLOCK_LENGTH / 2);
