 ...
 */
static int x509_crl_get_version(unsigned char **p, const unsigned char *end,
                                int *ver) {
  int ret;
 ...
 */
static int x509_get_crl_ext(unsigned char **p, const unsigned char *end,
  ...
  while (*p < end) {
    ...
    if ((ret = mbedtls_asn1_get_tag(p, end_ext_data, &len, MBEDTLS_ASN1_OID)) !=
        0) {
      return (MBEDTLS_ERR_X509_INVALID_EXTENSIONS + ret);
    }
    *p += len;
...
static int x509_get_entries(unsigned char **p, const unsigned char *end,
  ...
  while (*p < end) {
    ...
    if ((ret = mbedtls_x509_get_time(p, end2, &cur_entry->revocation_date)) !=
        0)
      return (ret);
...
int mbedtls_x509_crl_parse_der(mbedtls_x509_crl *chain,
  ...
  if ((ret = mbedtls_x509_get_sig_alg(&crl->sig_oid, &sig_params1, &crl->sig_md,
                                      &crl->sig_pk, &crl->sig_opts)) != 0) {
    mbedtls_x509_crl_free(crl);
