...
/* DRBG function */
static inline int ecp_drbg_random(void *p_rng, unsigned char *output,
                                  size_t output_len) {
 ...
 */
static int ecp_ctr_drbg_null_entropy(void *ctx, unsigned char *out,
                                     size_t len) {
  (void)ctx;
  memset(out, 0, len);
  return (0);
...
static int ecp_drbg_random(void *p_rng, unsigned char *output,
                           size_t output_len) {
...
int mbedtls_ecp_check_budget(const mbedtls_ecp_group *grp,
                             mbedtls_ecp_restart_ctx *rs_ctx, unsigned ops) {
...
static int ecp_comb_recode_scalar(const mbedtls_ecp_group *grp,
                                  const mbedtls_mpi *m,
                                  unsigned char k[COMB_MAX_D + 1], size_t d,
                                  unsigned char w,
 ...
 */
static int
ecp_mul_comb_after_precomp(const mbedtls_ecp_group *grp, mbedtls_ecp_point *R,
                           const mbedtls_mpi *m, const mbedtls_ecp_point *T,
                           unsigned char T_size, unsigned char w, size_t d,
                           int (*f_rng)(void *, unsigned char *, size_t),
                           void *p_rng, mbedtls_ecp_restart_ctx *rs_ctx) {
  int ret;
  unsigned char parity_trick;
...
static int ecp_mul_comb(mbedtls_ecp_group *grp, mbedtls_ecp_point *R,
  ...
  MBEDTLS_MPI_CHK(ecp_mul_comb_after_precomp(grp, R, m, T, T_size, w, d, f_rng,
                                             p_rng, rs_ctx));
...
static int mbedtls_ecp_mul_shortcuts(mbedtls_ecp_group *grp,
  ...
  if (mbedtls_mpi_cmp_int(m, 1) == 0) {
    ...
    MBEDTLS_MPI_CHK(
        mbedtls_ecp_mul_restartable(grp, R, m, P, NULL, NULL, rs_ctx));
  }
...
static const uint8_t test_kdf_out[40] = {
    0x3e, 0xf6, 0xda, 0xf9, 0x51, 0x60, 0x70, 0x5f, 0xdf, 0x21,
    0xcd, 0xab, 0xac, 0x25, 0x7b, 0x05, 0xfe, 0xc1, 0xab, 0x7c,
    0xc9, 0x68, 0x43, 0x25, 0x8a, 0xfc, 0x40, 0x6e, 0x5b, 0xf7,
    0x98, 0x27, 0x10, 0xfa, 0x7b, 0x93, 0x52, 0xd4, 0x16, 0xaa,
};
...
static const uint8_t test_kdf_out[40] = {
    0x7d, 0xf6, 0x41, 0xf8, 0x3c, 0x47, 0xdc, 0x28, 0x5f, 0x7f,
    0xaa, 0xde, 0x05, 0x64, 0xd6, 0x25, 0x00, 0x6a, 0x47, 0xd9,
    0x1e, 0xa4, 0xa0, 0x8c, 0xd7, 0xf7, 0x0c, 0x99, 0xaa, 0xa0,
    0x72, 0x66, 0x69, 0x0e, 0x25, 0xaa, 0xa1, 0x63, 0x14, 0x79,
};
...
static int ecp_kdf_self_test(void) {
  ...
  MBEDTLS_MPI_CHK(
      mbedtls_mpi_read_binary(&scalar, test_kdf_z, sizeof(test_kdf_z)));
