...
#define CTR_CRYPT(dst, src, len)                                               \
  do {                                                                         \
    if ((ret = mbedtls_cipher_update(&ctx->cipher_ctx, ctr, 16, b, &olen)) !=  \
        0) {                                                                   \
      return (ret);                                                            \
    }                                                                          \
                                                                               \
...
int mbedtls_ccm_star_auth_decrypt(mbedtls_ccm_context *ctx, size_t length,
  ...
  if ((ret = ccm_auth_crypt(ctx, CCM_DECRYPT, length, iv, iv_len, add, add_len,
 ...
 */
static const unsigned char key[] = {0x40, 0x41, 0x42, 0x43, 0x44, 0x45,
                                    0x46, 0x47, 0x48, 0x49, 0x4a, 0x4b,
                                    0x4c, 0x4d, 0x4e, 0x4f};

static const unsigned char iv[] = {0x10, 0x11, 0x12, 0x13, 0x14, 0x15,
                                   0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b};

static const unsigned char ad[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06,
...
int mbedtls_ccm_self_test(int verbose) {
  ...
  if (mbedtls_ccm_setkey(&ctx, MBEDTLS_CIPHER_ID_AES, key, 8 * sizeof key) !=
      0) {
    if (verbose != 0)
  ...
  for (i = 0; i < NB_TESTS; i++) {
    ...
    ret = mbedtls_ccm_encrypt_and_tag(&ctx, msg_len[i], iv, iv_len[i], ad,
                                      add_len[i], plaintext, ciphertext,
    ...
    ret = mbedtls_ccm_auth_decrypt(&ctx, msg_len[i], iv, iv_len[i], ad,
                                   add_len[i], ciphertext, plaintext,
                                   ciphertext + msg_len[i], tag_len[i]);
