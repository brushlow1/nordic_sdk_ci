...
void mbedtls_strerror(int ret, char *buf, size_t buflen) {
  ...
  if (ret & 0xFF80) {
    ...
    if (use_ret == -(MBEDTLS_ERR_ECP_FEATURE_UNAVAILABLE))
      mbedtls_snprintf(buf, buflen,
                       "ECP - The requested feature is not available, for "
                       "example, the requested curve is not supported");
    ...
    if (use_ret == -(MBEDTLS_ERR_ECP_IN_PROGRESS))
      mbedtls_snprintf(buf, buflen,
                       "ECP - Operation in progress, call again with the same "
    ...
    if (use_ret == -(MBEDTLS_ERR_PEM_PASSWORD_MISMATCH))
      mbedtls_snprintf(buf, buflen,
                       "PEM - Given private key password does not allow for "
                       "correct decryption");
    ...
    if (use_ret == -(MBEDTLS_ERR_PK_PASSWORD_MISMATCH))
      mbedtls_snprintf(buf, buflen,
                       "PK - Given private key password does not allow for "
                       "correct decryption");
    if (use_ret == -(MBEDTLS_ERR_PK_INVALID_PUBKEY))
      mbedtls_snprintf(buf, buflen,
                       "PK - The pubkey tag or value is invalid (only RSA and "
                       "EC are supported)");
    ...
    if (use_ret == -(MBEDTLS_ERR_PK_UNKNOWN_NAMED_CURVE))
      mbedtls_snprintf(buf, buflen,
                       "PK - Elliptic curve is unsupported (only NIST curves "
                       "are supported)");
    ...
    if (use_ret == -(MBEDTLS_ERR_PKCS12_PASSWORD_MISMATCH))
      mbedtls_snprintf(buf, buflen,
                       "PKCS12 - Given private key password does not allow for "
                       "correct decryption");
    ...
    if (use_ret == -(MBEDTLS_ERR_PKCS5_PASSWORD_MISMATCH))
      mbedtls_snprintf(buf, buflen,
                       "PKCS5 - Given private key password does not allow for "
                       "correct decryption");
    ...
    if (use_ret == -(MBEDTLS_ERR_RSA_UNSUPPORTED_OPERATION))
      mbedtls_snprintf(buf, buflen,
                       "RSA - The implementation does not offer the requested "
                       "operation, for example, because of security violations "
    ...
    if (use_ret == -(MBEDTLS_ERR_SSL_NO_CLIENT_CERTIFICATE))
      mbedtls_snprintf(buf, buflen,
                       "SSL - No client certification received from the "
                       "client, but required by the authentication mode");
    if (use_ret == -(MBEDTLS_ERR_SSL_CERTIFICATE_TOO_LARGE))
      mbedtls_snprintf(buf, buflen,
                       "SSL - Our own certificate(s) is/are too large to send "
                       "in an SSL message");
    ...
    if (use_ret == -(MBEDTLS_ERR_SSL_PEER_CLOSE_NOTIFY))
      mbedtls_snprintf(buf, buflen,
                       "SSL - The peer notified us that the connection is "
                       "going to be closed");
    ...
    if (use_ret == -(MBEDTLS_ERR_SSL_BAD_HS_CERTIFICATE_REQUEST))
      mbedtls_snprintf(buf, buflen,
                       "SSL - Processing of the CertificateRequest handshake "
                       "message failed");
    ...
    if (use_ret == -(MBEDTLS_ERR_SSL_BAD_HS_CLIENT_KEY_EXCHANGE_RP))
      mbedtls_snprintf(buf, buflen,
                       "SSL - Processing of the ClientKeyExchange handshake "
    ...
    if (use_ret == -(MBEDTLS_ERR_SSL_PK_TYPE_MISMATCH))
      mbedtls_snprintf(buf, buflen,
                       "SSL - Public key type mismatch (eg, asked for RSA key "
    ...
    if (use_ret == -(MBEDTLS_ERR_SSL_INTERNAL_ERROR))
      mbedtls_snprintf(buf, buflen,
                       "SSL - Internal error (eg, unexpected failure in "
                       "lower-level module)");
    ...
    if (use_ret == -(MBEDTLS_ERR_SSL_NO_USABLE_CIPHERSUITE))
      mbedtls_snprintf(buf, buflen,
                       "SSL - None of the common ciphersuites is usable (eg, "
                       "no suitable certificate, see debug messages)");
    if (use_ret == -(MBEDTLS_ERR_SSL_WANT_READ))
      mbedtls_snprintf(buf, buflen,
                       "SSL - No data of requested type currently available on "
    ...
    if (use_ret == -(MBEDTLS_ERR_SSL_CONTINUE_PROCESSING))
      mbedtls_snprintf(buf, buflen,
                       "SSL - Internal-only message signaling that further "
    ...
    if (use_ret == -(MBEDTLS_ERR_X509_FEATURE_UNAVAILABLE))
      mbedtls_snprintf(buf, buflen,
                       "X509 - Unavailable feature, e.g. RSA "
                       "hashing/encryption combination");
    if (use_ret == -(MBEDTLS_ERR_X509_UNKNOWN_OID))
      mbedtls_snprintf(buf, buflen, "X509 - Requested OID is unknown");
    if (use_ret == -(MBEDTLS_ERR_X509_INVALID_FORMAT))
      mbedtls_snprintf(buf, buflen,
                       "X509 - The CRT/CRL/CSR format is invalid, e.g. "
    ...
    if (use_ret == -(MBEDTLS_ERR_X509_SIG_MISMATCH))
      mbedtls_snprintf(buf, buflen,
                       "X509 - Signature algorithms do not match. (see \\c "
                       "::mbedtls_x509_crt sig_oid)");
    if (use_ret == -(MBEDTLS_ERR_X509_CERT_VERIFY_FAILED))
      mbedtls_snprintf(buf, buflen,
                       "X509 - Certificate verification failed, e.g. CRL, CA "
                       "or signature check failed");
    ...
    if (use_ret == -(MBEDTLS_ERR_X509_FATAL_ERROR))
      mbedtls_snprintf(buf, buflen,
                       "X509 - A fatal error occurred, eg the chain is too "
                       "long or the vrfy callback failed");
  ...
  if (use_ret == -(MBEDTLS_ERR_AES_FEATURE_UNAVAILABLE))
    mbedtls_snprintf(buf, buflen,
                     "AES - Feature not available. For example, an unsupported "
                     "AES key size");
  ...
  if (use_ret == -(MBEDTLS_ERR_ARIA_FEATURE_UNAVAILABLE))
    mbedtls_snprintf(buf, buflen,
                     "ARIA - Feature not available. For example, an "
                     "unsupported ARIA key size");
  ...
  if (use_ret == -(MBEDTLS_ERR_MPI_NEGATIVE_VALUE))
    mbedtls_snprintf(buf, buflen,
                     "BIGNUM - The input arguments are negative or result in "
                     "illegal output");
  if (use_ret == -(MBEDTLS_ERR_MPI_DIVISION_BY_ZERO))
    mbedtls_snprintf(buf, buflen,
                     "BIGNUM - The input argument for division is zero, which "
                     "is not allowed");
  ...
  if (use_ret == -(MBEDTLS_ERR_CHACHA20_FEATURE_UNAVAILABLE))
    mbedtls_snprintf(buf, buflen,
                     "CHACHA20 - Feature not available. For example, s part of "
  ...
  if (use_ret == -(MBEDTLS_ERR_CHACHAPOLY_BAD_STATE))
    mbedtls_snprintf(buf, buflen,
                     "CHACHAPOLY - The requested operation is not permitted in "
                     "the current state");
  ...
  if (use_ret == -(MBEDTLS_ERR_POLY1305_FEATURE_UNAVAILABLE))
    mbedtls_snprintf(buf, buflen,
                     "POLY1305 - Feature not available. For example, s part of "
                     "the API is not implemented");
