...
#if defined(MBEDTLS_TEST_NULL_ENTROPY)
  mbedtls_entropy_add_source(ctx, mbedtls_null_entropy_poll, NULL, 1,
...
static int entropy_gather_internal(mbedtls_entropy_context *ctx) {
  ...
  for (i = 0; i < ctx->source_count; i++) {
    ...
    if ((ret = ctx->source[i].f_source(ctx->source[i].p_source, buf,
                                       MBEDTLS_ENTROPY_MAX_GATHER, &olen)) !=
        0) {
      goto cleanup;
    }
...
int mbedtls_entropy_source_self_test(int verbose) {
  ...
  if ((ret = mbedtls_entropy_source_self_test_check_bits(buf0, sizeof(buf0))) !=
      0)
    goto cleanup;
  if ((ret = mbedtls_entropy_source_self_test_check_bits(buf1, sizeof(buf1))) !=
      0)
    goto cleanup;
