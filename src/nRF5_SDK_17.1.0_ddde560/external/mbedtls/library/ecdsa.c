...
sign:
  ...
  if (f_rng_blind != NULL)
    ...
    ret = ecdsa_sign_restartable(
        grp, r, s, d, buf, blen, mbedtls_hmac_drbg_random, p_rng,
        mbedtls_hmac_drbg_random, p_rng_blind_det, rs_ctx);
...
muladd:
   ...
   * Step 5: R = u1 G + u2 Q
   */
  MBEDTLS_MPI_CHK(mbedtls_ecp_muladd_restartable(grp, &R, pu1, &grp->G, pu2, Q,
                                                 ECDSA_RS_ECP));
...
int mbedtls_ecdsa_write_signature_restartable(
    mbedtls_ecdsa_context *ctx, mbedtls_md_type_t md_alg,
...
#else
  /* Use the same RNG for both blinding and ephemeral key generation */
  MBEDTLS_MPI_CHK(ecdsa_sign_restartable(&ctx->grp, &r, &s, &ctx->d, hash, hlen,
