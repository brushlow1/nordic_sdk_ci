...
int mbedtls_ssl_ticket_setup(mbedtls_ssl_ticket_context *ctx,
                             ...
                             void *p_rng, mbedtls_cipher_type_t cipher,
                             uint32_t lifetime) {
  int ret;
...
static int ssl_load_session(mbedtls_ssl_session *session,
  ...
  if (cert_len == 0) {
    ...
    if ((ret = mbedtls_x509_crt_parse_der(session->peer_cert, p, cert_len)) !=
        0) {
...
int mbedtls_ssl_ticket_write(void *p_ticket, const mbedtls_ssl_session *session,
  ...
  *ticket_lifetime = ctx->ticket_lifetime;
  ...
  /* Dump session state */
  if ((ret = ssl_save_session(session, state, end - state, &clear_len)) != 0 ||
  ...
  if ((ret = mbedtls_cipher_auth_encrypt(
           ...
           key_name, TICKET_ADD_DATA_LEN, state, clear_len, state, &ciph_len,
 ...
 */
int mbedtls_ssl_ticket_parse(void *p_ticket, mbedtls_ssl_session *session,
  ...
  if ((ret = mbedtls_cipher_auth_decrypt(
           &key->ctx, iv, TICKET_IV_BYTES,
           /* Additional data: key name, IV and length */
           key_name, TICKET_ADD_DATA_LEN, ticket, enc_len, ticket, &clear_len,
           tag, TICKET_AUTH_TAG_BYTES)) != 0) {
