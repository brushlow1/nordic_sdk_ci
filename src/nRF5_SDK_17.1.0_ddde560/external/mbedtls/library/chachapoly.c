...
static int chachapoly_crypt_and_tag(
    mbedtls_chachapoly_context *ctx, mbedtls_chachapoly_mode_t mode,
    size_t length, const unsigned char nonce[12], const unsigned char *aad,
    size_t aad_len, const unsigned char *input, unsigned char *output,
...
int mbedtls_chachapoly_self_test(int verbose) {
  ...
  for (i = 0U; i < 1U; i++) {
    ...
    ret = mbedtls_chachapoly_encrypt_and_tag(
        &ctx, test_input_len[i], test_nonce[i], test_aad[i], test_aad_len[i],
