...
int mbedtls_aes_setkey_dec(mbedtls_aes_context *ctx, const unsigned char *key,
  ...
  *RK++ = *SK++;
  ...
  for (i = ctx->nr - 1, SK -= 8; i > 0; i--, SK -= 8) {
    for (j = 0; j < 4; j++, SK++) {
      *RK++ = AES_RT0(FSb[(*SK) & 0xFF]) ^ AES_RT1(FSb[(*SK >> 8) & 0xFF]) ^
...
#define AES_RROUND(X0, X1, X2, X3, Y0, Y1, Y2, Y3)                             \
  do {                                                                         \
    (X0) = *RK++ ^ AES_RT0(((Y0)) & 0xFF) ^ AES_RT1(((Y3) >> 8) & 0xFF) ^      \
           AES_RT2(((Y2) >> 16) & 0xFF) ^ AES_RT3(((Y1) >> 24) & 0xFF);        \
                                                                               \
    (X1) = *RK++ ^ AES_RT0(((Y1)) & 0xFF) ^ AES_RT1(((Y0) >> 8) & 0xFF) ^      \
           AES_RT2(((Y3) >> 16) & 0xFF) ^ AES_RT3(((Y2) >> 24) & 0xFF);        \
                                                                               \
    (X2) = *RK++ ^ AES_RT0(((Y2)) & 0xFF) ^ AES_RT1(((Y1) >> 8) & 0xFF) ^      \
           AES_RT2(((Y0) >> 16) & 0xFF) ^ AES_RT3(((Y3) >> 24) & 0xFF);        \
                                                                               \
    (X3) = *RK++ ^ AES_RT0(((Y3)) & 0xFF) ^ AES_RT1(((Y2) >> 8) & 0xFF) ^      \
           AES_RT2(((Y1) >> 16) & 0xFF) ^ AES_RT3(((Y0) >> 24) & 0xFF);        \
  } while (0)
...
#define GET_UINT64_LE(n, b, i)                                                 \
  {                                                                            \
    (n) = ((uint64_t)(b)[(i) + 7] << 56) | ((uint64_t)(b)[(i) + 6] << 48) |    \
          ((uint64_t)(b)[(i) + 5] << 40) | ((uint64_t)(b)[(i) + 4] << 32) |    \
          ((uint64_t)(b)[(i) + 3] << 24) | ((uint64_t)(b)[(i) + 2] << 16) |    \
...
int mbedtls_aes_crypt_xts(mbedtls_aes_xts_context *ctx, int mode, size_t length,
  ...
  /* Compute the tweak. */
  ret =
      mbedtls_aes_crypt_ecb(&ctx->tweak, MBEDTLS_AES_ENCRYPT, data_unit, tweak);
  if (ret != 0)
    return (ret);
...
int mbedtls_aes_self_test(int verbose) {
  ...
  for (i = 0; i < 6; i++) {
    ...
    ret = mbedtls_aes_crypt_ctr(&ctx, len, &offset, nonce_counter, stream_block,
                                buf, buf);
    if (ret != 0)
      goto exit;
