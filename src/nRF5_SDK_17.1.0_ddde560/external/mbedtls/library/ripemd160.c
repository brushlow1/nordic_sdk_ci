 ...
 */
int mbedtls_ripemd160_ret(const unsigned char *input, size_t ilen,
                          unsigned char output[20]) {
  int ret;
...
static const size_t ripemd160_test_strlen[TESTS] = {0,  1,  3,  14,
                                                    26, 56, 62, 80};
...
int mbedtls_ripemd160_self_test(int verbose) {
  ...
  for (i = 0; i < TESTS; i++) {
    ...
    ret = mbedtls_ripemd160_ret(ripemd160_test_str[i], ripemd160_test_strlen[i],
                                output);
    if (ret != 0)
      goto fail;
