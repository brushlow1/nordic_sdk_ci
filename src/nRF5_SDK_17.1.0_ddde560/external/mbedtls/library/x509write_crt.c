...
int mbedtls_x509write_crt_set_basic_constraints(mbedtls_x509write_cert *ctx,
  ...
  return (mbedtls_x509write_crt_set_extension(
      ...
      MBEDTLS_OID_SIZE(MBEDTLS_OID_BASIC_CONSTRAINTS), 0,
...
int mbedtls_x509write_crt_set_authority_key_identifier(
  ...
  MBEDTLS_ASN1_CHK_ADD(len, mbedtls_asn1_write_len(&c, buf, len));
  MBEDTLS_ASN1_CHK_ADD(
      len, mbedtls_asn1_write_tag(&c, buf, MBEDTLS_ASN1_CONTEXT_SPECIFIC | 0));
...
int mbedtls_x509write_crt_set_key_usage(mbedtls_x509write_cert *ctx,
  ...
  ret = mbedtls_x509write_crt_set_extension(
      ctx, MBEDTLS_OID_KEY_USAGE, MBEDTLS_OID_SIZE(MBEDTLS_OID_KEY_USAGE), 1, c,
      (size_t)ret);
  if (ret != 0)
    return (ret);
...
static int x509_write_time(unsigned char **p, unsigned char *start,
  ...
  if (t[0] == '2' && t[1] == '0' && t[2] < '5') {
    ...
    MBEDTLS_ASN1_CHK_ADD(len, mbedtls_asn1_write_len(p, start, len));
    MBEDTLS_ASN1_CHK_ADD(
        len, mbedtls_asn1_write_tag(p, start, MBEDTLS_ASN1_GENERALIZED_TIME));
  }
...
#if defined(MBEDTLS_PEM_WRITE_C)
int mbedtls_x509write_crt_pem(mbedtls_x509write_cert *crt, unsigned char *buf,
                              size_t size,
                              int (*f_rng)(void *, unsigned char *, size_t),
                              void *p_rng) {
  int ret;
