 ...
 */
static int
ssl_ciphersuite_match(mbedtls_ssl_context *ssl, int suite_id,
                      const mbedtls_ssl_ciphersuite_t **ciphersuite_info) {
  ...
  if (ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_3) {
    ...
    if (sig_type != MBEDTLS_PK_NONE &&
        mbedtls_ssl_sig_hash_set_find(&ssl->handshake->hash_algs, sig_type) ==
            MBEDTLS_MD_NONE) {
...
static int ssl_parse_client_hello_v2(mbedtls_ssl_context *ssl) {
  ...
  MBEDTLS_SSL_DEBUG_MSG(
      3, ("client hello v2, max. version: [%d:%d]", buf[3], buf[4]));
  ...
  ssl->minor_ver =
      (buf[4] <= ssl->conf->max_minor_ver) ? buf[4] : ssl->conf->max_minor_ver;
  ...
  MBEDTLS_SSL_DEBUG_BUF(3, "client hello, session id", buf + 6 + ciph_len,
                        sess_len);
  ...
  for (i = 0, p = buf + 6; i < ciph_len; i += 3, p += 3) {
    if (p[0] == 0 && p[1] == 0 &&
        p[2] == MBEDTLS_SSL_EMPTY_RENEGOTIATION_INFO) {
  ...
  for (i = 0, p = buf + 6; i < ciph_len; i += 3, p += 3) {
    if (p[0] == 0 &&
        p[1] ==
            (unsigned char)((MBEDTLS_SSL_FALLBACK_SCSV_VALUE >> 8) & 0xff) &&
...
#endif
    {
      if (p[0] != 0 || p[1] != ((ciphersuites[i] >> 8) & 0xFF) ||
...
read_record_header:
  ...
  MBEDTLS_SSL_DEBUG_MSG(
      3, ("client hello v3, protocol version: [%d:%d]", buf[1], buf[2]));
  ...
  if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
    ...
    MBEDTLS_SSL_DEBUG_BUF(3, "client hello, cookie", buf + cookie_offset + 1,
                          cookie_len);
  ...
  if ((ssl->major_ver != 3) || (ssl->minor_ver != 0)) {
    ...
    defined(MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED)
      ...
      default:
        MBEDTLS_SSL_DEBUG_MSG(
            3, ("unknown extension found: %d (ignoring)", ext_id));
      }
  ...
  if (ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_3) {
    ...
    if (sig_alg != MBEDTLS_PK_NONE) {
      mbedtls_md_type_t md_alg =
          mbedtls_ssl_sig_hash_set_find(&ssl->handshake->hash_algs, sig_alg);
...
static int ssl_write_server_hello(mbedtls_ssl_context *ssl) {
  ...
  MBEDTLS_SSL_DEBUG_MSG(
      3, ("server hello, chosen version: [%d:%d]", buf[4], buf[5]));
  ...
  *p++ = (unsigned char)(ssl->session_negotiate->compression);
  ...
  if ((ssl->major_ver != 3) || (ssl->minor_ver != 0)) {
    ...
    MBEDTLS_SSL_DEBUG_MSG(
        3, ("server hello, total extension length: %d", ext_len));
...
static int ssl_prepare_server_key_exchange(mbedtls_ssl_context *ssl,
  ...
  if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECJPAKE) {
    ...
    ret = mbedtls_ecjpake_write_round_two(
        ...
        MBEDTLS_SSL_OUT_CONTENT_LEN - ssl->out_msglen, &len, ssl->conf->f_rng,
        ssl->conf->p_rng);
    if (ret != 0) {
  ...
  curve_matching_done:
    ...
    if ((ret = mbedtls_ecdh_make_params(
             ...
             MBEDTLS_SSL_OUT_CONTENT_LEN - ssl->out_msglen, ssl->conf->f_rng,
             ssl->conf->p_rng)) != 0) {
  ...
  if (mbedtls_ssl_ciphersuite_uses_server_signature(ciphersuite_info)) {
    ...
    if (md_alg == MBEDTLS_MD_NONE) {
      hashlen = 36;
      ret = mbedtls_ssl_get_key_exchange_md_ssl_tls(ssl, hash, dig_signed,
                                                    dig_signed_len);
      if (ret != 0)
        return (ret);
    } else
     ...
     * ssl->out_msglen. */
    if ((ret =
             mbedtls_pk_sign(mbedtls_ssl_own_key(ssl), md_alg, hash, hashlen,
...
static int ssl_resume_decrypt_pms(mbedtls_ssl_context *ssl,
                                  unsigned char *peer_pms, size_t *peer_pmslen,
                                  size_t peer_pmssize) {
...
static int ssl_decrypt_encrypted_pms(mbedtls_ssl_context *ssl,
  ...
  if (ssl->conf->f_async_decrypt_start != NULL) {
    ret = ssl->conf->f_async_decrypt_start(ssl, mbedtls_ssl_own_cert(ssl), p,
                                           len);
    switch (ret) {
    case MBEDTLS_ERR_SSL_HW_ACCEL_FALLTHROUGH:
...
static int ssl_parse_encrypted_pms(mbedtls_ssl_context *ssl,
  ...
  mbedtls_ssl_write_version(ssl->handshake->max_major_ver,
                            ssl->handshake->max_minor_ver, ssl->conf->transport,
                            ver);
...
static int ssl_parse_certificate_verify(mbedtls_ssl_context *ssl) {
  ...
  if ((ret = mbedtls_pk_verify(&ssl->session_negotiate->peer_cert->pk, md_alg,
                               hash_start, hashlen, ssl->in_msg + i,
                               sig_len)) != 0) {
