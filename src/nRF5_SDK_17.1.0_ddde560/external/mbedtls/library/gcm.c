...
int mbedtls_gcm_self_test(int verbose) {
  ...
  for (j = 0; j < 3; j++) {
    ...
    for (i = 0; i < MAX_TESTS; i++) {
      ...
      ret = mbedtls_gcm_crypt_and_tag(&ctx, MBEDTLS_GCM_ENCRYPT, pt_len[i],
      ...
      ret = mbedtls_gcm_crypt_and_tag(&ctx, MBEDTLS_GCM_DECRYPT, pt_len[i],
                                      iv[iv_index[i]], iv_len[i],
                                      additional[add_index[i]], add_len[i],
      ...
      ret = mbedtls_gcm_starts(&ctx, MBEDTLS_GCM_ENCRYPT, iv[iv_index[i]],
      ...
      if (pt_len[i] > 32) {
        ...
        ret =
            mbedtls_gcm_update(&ctx, rest_len, pt[pt_index[i]] + 32, buf + 32);
        if (ret != 0)
          goto exit;
      } else {
