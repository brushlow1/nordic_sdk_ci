 ...
 */
static void aria_rot128(uint32_t r[4], const uint32_t a[4], const uint32_t b[4],
                        uint8_t n) {
  uint8_t i, j;
  uint32_t t, u;
...
static const uint8_t aria_test1_ecb_ct[3]
                                      [MBEDTLS_ARIA_BLOCKSIZE] = // ciphertext
...
int mbedtls_aria_self_test(int verbose) {
  ...
  for (i = 0; i < 3; i++) {
    ...
    mbedtls_aria_crypt_cbc(&ctx, MBEDTLS_ARIA_ENCRYPT, 48, iv, aria_test2_pt,
                           buf);
