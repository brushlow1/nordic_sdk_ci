...
static void platform_free_uninit(void *ptr) { ((void)ptr); }
 ...
 * Make dummy function to prevent NULL pointer dereferences
 */
static void platform_exit_uninit(int status) { ((void)status); }
...
int mbedtls_platform_set_time(
    mbedtls_time_t (*time_func)(mbedtls_time_t *timer)) {
 ...
 */
void mbedtls_platform_teardown(mbedtls_platform_context *ctx) { (void)ctx; }
