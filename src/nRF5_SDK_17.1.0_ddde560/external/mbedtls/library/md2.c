...
int mbedtls_internal_md2_process(mbedtls_md2_context *ctx) {
  ...
  for (i = 0; i < 16; i++) {
    ctx->cksum[i] =
        (unsigned char)(ctx->cksum[i] ^ PI_SUBST[ctx->buffer[i] ^ t]);
    t = ctx->cksum[i];
  }
 ...
 * MD2 process buffer
 */
int mbedtls_md2_update_ret(mbedtls_md2_context *ctx, const unsigned char *input,
                           size_t ilen) {
  int ret;
  size_t fill;
 ...
 */
int mbedtls_md2_ret(const unsigned char *input, size_t ilen,
...
#if !defined(MBEDTLS_DEPRECATED_REMOVED)
void mbedtls_md2(const unsigned char *input, size_t ilen,
                 unsigned char output[16]) {
...
static const unsigned char md2_test_sum[7][16] = {
    ...
    {0x32, 0xEC, 0x01, 0xEC, 0x4A, 0x6D, 0xAC, 0x72, 0xC0, 0xAB, 0x96, 0xFB,
     0x34, 0xC0, 0xB5, 0xD1},
    {0xDA, 0x85, 0x3B, 0x0D, 0x3F, 0x88, 0xD9, 0x9B, 0x30, 0x28, 0x3A, 0x69,
     0xE6, 0xDE, 0xD6, 0xBB},
