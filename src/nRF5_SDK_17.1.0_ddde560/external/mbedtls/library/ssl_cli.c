...
static int ssl_write_supported_elliptic_curves_ext(mbedtls_ssl_context *ssl,
  ...
  *p++ =
      (unsigned char)((MBEDTLS_TLS_EXT_SUPPORTED_ELLIPTIC_CURVES >> 8) & 0xFF);
  ...
  *q++ = (unsigned char)(n << 1);
  ...
  if (offer_compress) {
    ...
    *p++ = MBEDTLS_SSL_COMPRESS_NULL;
    ...
    MBEDTLS_SSL_DEBUG_MSG(
        3, ("client hello, compress alg.: %d", MBEDTLS_SSL_COMPRESS_NULL));
...
static int ssl_parse_encrypt_then_mac_ext(mbedtls_ssl_context *ssl,
                                          ...
                                          size_t len) {
  if (ssl->conf->encrypt_then_mac == MBEDTLS_SSL_ETM_DISABLED ||
      ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_0 || len != 0) {
...
static int ssl_parse_server_hello(mbedtls_ssl_context *ssl) {
  ...
  if (ssl->major_ver < ssl->conf->min_major_ver ||
    ...
    MBEDTLS_SSL_DEBUG_MSG(1,
                          ("server version out of bounds -  min: [%d:%d], "
                           "server: [%d:%d], max: [%d:%d]",
 ...
 */
static int ssl_write_encrypted_pms(mbedtls_ssl_context *ssl, size_t offset,
                                   size_t *olen, size_t pms_offset) {
  int ret;
...
static int ssl_parse_certificate_request(mbedtls_ssl_context *ssl) {
  ...
  if (ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_3) {
    ...
    for (i = 0; i < sig_alg_len; i += 2) {
      MBEDTLS_SSL_DEBUG_MSG(3, ("Supported Signature Algorithm found: %d,%d",
...
static int ssl_write_client_key_exchange(mbedtls_ssl_context *ssl) {
  ...
  if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_DHE_RSA) {
    ...
    ret = mbedtls_dhm_make_public(
        ...
        (int)mbedtls_mpi_size(&ssl->handshake->dhm_ctx.P), &ssl->out_msg[i], n,
        ssl->conf->f_rng, ssl->conf->p_rng);
    if (ret != 0) {
  ...
  ecdh_calc_secret:
    ...
    if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_PSK) {
      ...
      ret = mbedtls_dhm_make_public(
          &ssl->handshake->dhm_ctx,
          (int)mbedtls_mpi_size(&ssl->handshake->dhm_ctx.P), &ssl->out_msg[i],
          n, ssl->conf->f_rng, ssl->conf->p_rng);
      if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_dhm_make_public", ret);
        return (ret);
      }
