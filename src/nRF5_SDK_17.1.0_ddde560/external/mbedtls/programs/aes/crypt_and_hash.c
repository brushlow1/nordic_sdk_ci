/*
 *  \brief  Generic file encryption program using generic wrappers for
 * configured security.
 *
...
#define USAGE                                                                  \
  "\n  crypt_and_hash <mode> <input filename> <output filename> <cipher> "     \
  "<mbedtls_md> <key>\n"                                                       \
  "\n   <mode>: 0 = encrypt, 1 = decrypt\n"                                    \
  "\n  example: crypt_and_hash 0 file file.aes AES-128-CBC SHA1 "              \
  "hex:E76B2413958B00E193\n"                                                   \
  "\n"

#if !defined(MBEDTLS_CIPHER_C) || !defined(MBEDTLS_MD_C) ||                    \
    !defined(MBEDTLS_FS_IO)
int main(void) {
  mbedtls_printf("MBEDTLS_CIPHER_C and/or MBEDTLS_MD_C and/or MBEDTLS_FS_IO "
                 "not defined.\n");
  mbedtls_exit(0);
}
#else
...
int main(int argc, char *argv[]) {
  ...
  li_size.QuadPart = 0;
  li_size.LowPart =
      SetFilePointer((HANDLE)_get_osfhandle(_fileno(fin)), li_size.LowPart,
  ...
  if (mode == MODE_ENCRYPT) {
    ...
    for (offset = 0; offset < filesize;
      ...
      if (mbedtls_cipher_update(&cipher_ctx, buffer, ilen, output, &olen) !=
          0) {
  ...
  if (mode == MODE_DECRYPT) {
    ...
    if (cipher_info->mode != MBEDTLS_MODE_GCM &&
      ...
      mbedtls_fprintf(stderr,
                      "File content not a multiple of the block size (%d).\n",
                      mbedtls_cipher_get_block_size(&cipher_ctx));
      goto exit;
    }
