...
#if !defined(MBEDTLS_ENTROPY_C) || !defined(MBEDTLS_SSL_TLS_C) ||              \
...
#define USAGE_IO                                                               \
  ...
  "    ca_path=%%s          The path containing the top-level CA(s) you "      \
  "fully trust\n"                                                              \
  "                        default: \"\" (pre-loaded) (overrides ca_file)\n"   \
  "                        use \"none\" to skip loading any top-level CAs.\n"  \
  "    crt_file=%%s         Your own cert and chain (in bottom to top order, " \
  "top may be omitted)\n"                                                      \
  "                        default: see note after key_file2\n"                \
  "    key_file=%%s         default: see note after key_file2\n"               \
  "    crt_file2=%%s        Your second cert and chain (in bottom to top "     \
  "order, top may be omitted)\n"                                               \
  "                        default: see note after key_file2\n"                \
  "    key_file2=%%s        default: see note below\n"                         \
  "                        note: if neither crt_file/key_file nor "            \
  "crt_file2/key_file2 are used,\n"                                            \
  "                              preloaded certificate(s) and key(s) are "     \
  "used if available\n"                                                        \
...
#define USAGE_SSL_ASYNC                                                        \
  ...
  "    async_private_delay1=%%d  Asynchronous delay for key_file or "          \
  "preloaded key\n"                                                            \
  "    async_private_delay2=%%d  Asynchronous delay for key_file2 and sni\n"   \
  "                              default: -1 (not asynchronous)\n"             \
  "    async_private_error=%%d   Async callback error injection "              \
  "(default=0=none,\n"                                                         \
  "                              1=start, 2=cancel, 3=resume, negative=first " \
  "time only)"
#else
#define USAGE_SSL_ASYNC ""
...
#define USAGE_ECJPAKE "    ecjpake_pw=%%s       default: none (disabled)\n"
#else
...
#define USAGE2                                                                 \
  ...
  "                        options: 1 (send ca list), 0 (don't "               \
  "send)\n" USAGE_IO USAGE_SSL_ASYNC USAGE_SNI "\n" USAGE_PSK USAGE_ECJPAKE    \
  "\n"
#define USAGE3                                                                 \
  "    allow_legacy=%%d     default: (library default: no)\n" USAGE_RENEGO     \
  "    exchanges=%%d        default: 1\n"                                      \
  "\n" USAGE_TICKETS USAGE_CACHE USAGE_MAX_FRAG_LEN USAGE_TRUNC_HMAC           \
      USAGE_ALPN USAGE_EMS USAGE_ETM USAGE_CURVES "\n"
#define USAGE4                                                                 \
  ...
  "                        options: ssl3, tls1, tls1_1, tls1_2, dtls1, "       \
  "dtls1_2\n"                                                                  \
  "\n"                                                                         \
...
struct options {
  ...
  int async_private_delay1; /* number of times f_async_resume needs to be called
                               for key 1, or -1 for no async */
  int async_private_delay2; /* number of times f_async_resume needs to be called
                               for key 2, or -1 for no async */
...
int ssl_async_set_key(ssl_async_key_context_t *ctx, mbedtls_x509_crt *cert,
                      mbedtls_pk_context *pk, int pk_take_ownership,
                      unsigned delay) {
...
static int ssl_async_start(mbedtls_ssl_context *ssl, mbedtls_x509_crt *cert,
                           ssl_async_operation_type_t op_type,
                           mbedtls_md_type_t md_alg, const unsigned char *input,
                           size_t input_len) {
...
static int ssl_async_sign(mbedtls_ssl_context *ssl, mbedtls_x509_crt *cert,
                          mbedtls_md_type_t md_alg, const unsigned char *hash,
                          size_t hash_len) {
  return (ssl_async_start(ssl, cert, ASYNC_OP_SIGN, md_alg, hash, hash_len));
}

static int ssl_async_decrypt(mbedtls_ssl_context *ssl, mbedtls_x509_crt *cert,
                             const unsigned char *input, size_t input_len) {
  return (ssl_async_start(ssl, cert, ASYNC_OP_DECRYPT, MBEDTLS_MD_NONE, input,
                          input_len));
}

static int ssl_async_resume(mbedtls_ssl_context *ssl, unsigned char *output,
  ...
  case ASYNC_OP_DECRYPT:
    ret = mbedtls_pk_decrypt(key_slot->pk, ctx->input, ctx->input_len, output,
                             output_len, output_size, config_data->f_rng,
                             config_data->p_rng);
    break;
  case ASYNC_OP_SIGN:
    ret = mbedtls_pk_sign(key_slot->pk, ctx->md_alg, ctx->input, ctx->input_len,
                          output, output_len, config_data->f_rng,
                          config_data->p_rng);
    break;
  default:
    mbedtls_printf("Async resume (slot %u): unknown operation type %ld. This "
                   "shouldn't happen.\n",
...
#else
int idle(mbedtls_net_context *fd, mbedtls_timing_delay_context *timer,
         int idle_reason)
#endif
{
  int ret;
...
int main(int argc, char *argv[]) {
    ...
    *q++ = '\0';
    ...
    if (strcmp(p, "server_port") == 0)
      ...
      if (opt.buffer_size < 1 ||
          opt.buffer_size > MBEDTLS_SSL_MAX_CONTENT_LEN + 1)
        goto usage;
    } else if (strcmp(p, "response_size") == 0) {
      opt.response_size = atoi(q);
      if (opt.response_size < 0 ||
          opt.response_size > MBEDTLS_SSL_MAX_CONTENT_LEN)
        goto usage;
  ...
  if (opt.curves != NULL) {
    ...
    if (strcmp(p, "none") == 0) {
      ...
      while (i < CURVE_LIST_SIZE - 1 && *p != '\0') {
        ...
        if ((curve_cur = mbedtls_ecp_curve_info_from_name(q)) != NULL) {
          ...
          for (curve_cur = mbedtls_ecp_curve_list();
               curve_cur->grp_id != MBEDTLS_ECP_DP_NONE; curve_cur++) {
      ...
      if (i == CURVE_LIST_SIZE - 1 && *p != '\0') {
        mbedtls_printf("curves list too long, maximum %d", CURVE_LIST_SIZE - 1);
        goto exit;
      }
  ...
  if (opt.tickets == MBEDTLS_SSL_SESSION_TICKETS_ENABLED) {
    if ((ret = mbedtls_ssl_ticket_setup(&ticket_ctx, mbedtls_ctr_drbg_random,
                                        &ctr_drbg, MBEDTLS_CIPHER_AES_256_GCM,
  ...
  if (opt.async_operations[0] != '-') {
    ...
    ssl_async_keys.inject_error =
        (opt.async_private_error < 0 ? -opt.async_private_error
                                     : opt.async_private_error);
    ssl_async_keys.f_rng = mbedtls_ctr_drbg_random;
    ssl_async_keys.p_rng = &ctr_drbg;
    mbedtls_ssl_conf_async_private_cb(&conf, sign, decrypt, ssl_async_resume,
                                      ssl_async_cancel, &ssl_async_keys);
  }
#endif /* MBEDTLS_SSL_ASYNC_PRIVATE */
#endif /* MBEDTLS_X509_CRT_PARSE_C */
    ...
    do {
      ...
      if (mbedtls_ssl_get_bytes_avail(&ssl) == 0) {
        ...
        mbedtls_printf(" %u bytes read (%u + %u)\n\n%s\n", ori_len + extra_len,
                       ori_len, extra_len, (char *)larger_buf);
