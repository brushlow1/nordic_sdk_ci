...
#define MBEDTLS_X509_BADCERT_CN_MISMATCH                                       \
  0x04 /**< The certificate Common Name (CN) does not match with the expected  \
          CN. */
...
#define MBEDTLS_X509_BADCERT_BAD_PK                                            \
  0x8000 /**< The certificate is signed with an unacceptable PK alg (eg RSA vs \
            ECDSA). */
...
#define MBEDTLS_X509_BADCRL_BAD_KEY                                            \
  0x080000 /**< The CRL is signed with an unacceptable key (eg bad curve, RSA  \
              too short). */
 ...
 *                 terminated nul byte), or a negative error code.
 */
int mbedtls_x509_serial_gets(char *buf, size_t size,
...
int mbedtls_x509_get_rsassa_pss_params(const mbedtls_x509_buf *params,
                                       ...
                                       int *salt_len);
#endif
int mbedtls_x509_get_sig(unsigned char **p, const unsigned char *end,
...
int mbedtls_x509_write_sig(unsigned char **p, unsigned char *start,
                           const char *oid, size_t oid_len, unsigned char *sig,
                           size_t size);
