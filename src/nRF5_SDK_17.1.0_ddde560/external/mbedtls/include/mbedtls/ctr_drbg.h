...
#define MBEDTLS_ERR_CTR_DRBG_FILE_IO_ERROR                                     \
  -0x003A /**< Read or write error in file. */
...
#define MBEDTLS_CTR_DRBG_SEEDLEN                                               \
  (MBEDTLS_CTR_DRBG_KEYSIZE +                                                  \
   MBEDTLS_CTR_DRBG_BLOCKSIZE) /**< The seed length, calculated as (counter +  \
                                  AES key). */
...
/**
 * \brief               This function resets CTR_DRBG context to the state
 * immediately after initial call of mbedtls_ctr_drbg_init().
 *
