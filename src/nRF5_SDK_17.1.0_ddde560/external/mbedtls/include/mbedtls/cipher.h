...
#define MBEDTLS_ERR_CIPHER_ALLOC_FAILED                                        \
  -0x6180 /**< Failed to allocate memory. */
...
#define MBEDTLS_ERR_CIPHER_AUTH_FAILED                                         \
  -0x6300 /**< Authentication failed (for AEAD modes). */
...
#define MBEDTLS_ERR_CIPHER_HW_ACCEL_FAILED                                     \
  -0x6400 /**< Cipher hardware accelerator failed. */
...
typedef enum {
  ...
  MBEDTLS_CIPHER_ARIA_256_CBC, /**< Aria cipher with 256-bit key and CBC mode.
                                */
  MBEDTLS_CIPHER_ARIA_128_CFB128, /**< Aria cipher with 128-bit key and CFB-128
                                     mode. */
  MBEDTLS_CIPHER_ARIA_192_CFB128, /**< Aria cipher with 192-bit key and CFB-128
                                     mode. */
  MBEDTLS_CIPHER_ARIA_256_CFB128, /**< Aria cipher with 256-bit key and CFB-128
                                     mode. */
 ...
 *                      given \p cipher_name.
 * \return              \c NULL if the associated cipher information is not
 * found.
 */
...
/**
 * \brief               This function sets the key to use with the given
 * context.
 *
 ...
 * \param iv_len        The IV length for ciphers with variable-size IV.
 *                      This parameter is discarded by ciphers with fixed-size
 * IV. \param ad            The additional data to authenticate. This must be a
 ...
 * \param iv_len        The IV length for ciphers with variable-size IV.
 *                      This parameter is discarded by ciphers with fixed-size
 * IV. \param ad            The additional data to be authenticated. This must
 * be a readable buffer of at least \p ad_len Bytes. \param ad_len        The
 * length of \p ad. \param input         The buffer holding the input data. This
 * must be a readable buffer of at least \p ilen Bytes. \param ilen          The
 * length of the input data. \param output        The buffer for the output
 * data. This must be able to hold at least \p ilen Bytes. \param olen The
 * length of the output data, to be updated with the actual number of Bytes
 * written. This must not be \c NULL. \param tag           The buffer holding
 * the authentication tag. This must be a readable buffer of at least \p tag_len
 * Bytes. \param tag_len       The length of the authentication tag.
 *
 * \return              \c 0 on success.
 * \return              #MBEDTLS_ERR_CIPHER_BAD_INPUT_DATA on
 *                      parameter-verification failure.
