...
DWORD allocate_contiguous_clusters(/* Returns the first sector in LBA (0:error
                                      or not contiguous) */
                                   FIL *
  ...
  if (f_lseek(fp, 0) || !len) /* Check if the given parameters are valid */
    return 0;
  csz = 512UL * fp->fs->csize; /* Cluster size in unit of byte (assuming 512
                                  bytes/sector) */
  ...
  if (len == fp->fsize) {
    ...
    if (ncl == tcl) /* Is the file contiguous? */
      return clust2sect(
          fp->fs, fp->sclust); /* File is contiguous. Return the start sector */
  }
...
int main(void) {
  ...
  ... dr = disk_write(fil.fs->drv, Buff, org,
